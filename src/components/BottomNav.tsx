...
const BottomNav = ({ activeSection, onNavigate }: BottomNavProps) => {
  return (
    <nav className="xl:hidden fixed bottom-0 left-0 right-0 z-50 bg-card backdrop-blur-md border-t border-border/30 safe-area-bottom">
      <div className="flex items-center justify-evenly px-3 py-3 max-w-screen-md mx-auto gap-2">
        {navItems.map(({ id, icon: Icon, label }) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center gap-1 py-2 px-4 rounded-lg transition-all duration-200 min-w-[64px] ${
                isActive
